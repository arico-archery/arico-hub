...
1. **실제 데이터 입력 및 검증**
   ...
   - KOREA/ANGEL: CSV 업로드
   - 카탈로그 매칭은 **수동으로** (`/catalog` 에서 상품별 연결, 또는 주문 시 연결→자동 학습)
...
| 카탈로그 자동매칭 | **제거** (2026-07-17, 지안 결정) | 이름 유사도로는 정확도가 안 나옴. 카탈로그=일본어 / JVD=영어라 근본적으로 한계. 대신 **주문 시 사람이 고른 매칭을 학습**(`/api/orders` POST)하는 쪽만 남긴다 — 실제 거래로 검증된 매칭이라 신뢰도가 높다. 되살리려면 `git show` 로 `src/app/api/auto-match/route.ts` 참고 |
