...
## 5-2. SKU 3段連結（2026-08-13〜 構築中）

カタログは商品群単位なので、色・左右まで指定した発注には人の選び直しが要る。
その間にスマレジの SKU を挟んで精度を上げる構造を作っている。

```
カタログ（商品群） ──1:N──▶ スマレジ SKU（オプション別） ──1:1──▶ 仕入先バリエーション
```

- `SmaregiProduct.catalogId` … 受注取込・手書き受注のときに**自動記録**（`src/lib/sku-link.ts`）
- `SmaregiProduct.supplierProductId` … 発注対象。**必ず人が確定する**（自動設定は禁止）
- 確定する画面は2つ、どちらも同じ API を叩くので結果は同じ
  - `/sku-links` … 販売数の多い順に潰していく用
  - カタログ画面の行を展開（SkuPanel）… 商品群ごとに見る用
- リンクが無い SKU は現行方式（カタログ→仕入先の直結）にフォールバックする。
  だから確定した分だけ正確になり、途中で止めても壊れない。

進捗と残作業は `05-open-issues.md` を見ること。
