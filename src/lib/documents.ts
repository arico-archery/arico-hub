...
// 고객용 문서(청구서·견적서)에서 뺄 "재고·납기 안내" 태그.
//  取寄せ/取り寄せ(一部 포함)·即納·一部在庫あり·在庫限り·○月○日納品予定
// ※ 상품 정체성 태그(【バラ】【アウトレット】【カスタムオーダー】【2026】【2026年防具セット】
//    【旧デザイン】【2025Ver】【廃版】 등)는 상품명의 일부라 남긴다.
const DELIVERY_TAG = /【\s*(?:(?:一部)?お?取り?寄せ(?:商品)?|即納(?:商品)?|一部在庫あり|在庫限り|[^】]*納品予定)\s*】/g

// 고객용 문서 표기 정리 — 안내 태그 제거. 상품명 접두뿐 아니라
// 옵션값 안(예 "カラー : 【取寄せ商品】パープル")에도 섞여 있어 전역 제거.
//  예: 【取り寄せ商品】Diamond コンパウンドボウ → Diamond コンパウンドボウ
export function cleanDocText(s: string | null | undefined): string {
  return String(s || '')
    .replace(DELIVERY_TAG, '')
    .replace(/\s+/g, ' ')
    .trim()
}
