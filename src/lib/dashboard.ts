...
// 대시보드 집계 데이터 — 60초 캐시(Vercel Data Cache). 콜드스타트/반복 진입 시 즉시 응답.
// API 라우트(/api/dashboard)와 서버 컴포넌트(page.tsx) 양쪽에서 import해서 쓴다.
async function buildDashboard(range: string) {
  ...
  const totalUnpaid = unpaidOrders.reduce((acc, o) => acc + (o.totalAmountJpy - o.paidAmountJpy), 0)
...
export const getCachedDashboard = unstable_cache(buildDashboard, ['dashboard'], { revalidate: 60 })
