...
// 스마레지 SKU 의 옵션 문자열이 변형의 옵션값을 전부 담고 있으면 추천
function isGuess(sku: Sku, v: Variant): boolean {
  const hay = norm(`${sku.name} ${sku.size} ${sku.color}`)
    ...
    <div className="py-2 space-y-2">
      ...
      {skus.length === 0 ? (
        ...
        <div className="space-y-1">
          {skus.map(s => (
            <div key={s.id} className="flex items-start gap-2 text-xs bg-white dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 px-2.5 py-1.5">
      ...
      {/* 변형 선택 패널 (선택 중인 SKU 아래 한 번만 렌더) */}
      {pickFor != null && (
        <div className="bg-blue-50/60 dark:bg-blue-900/20 rounded-lg px-2.5 py-2">
          {!variants ? (
            <p className="text-xs text-gray-400">{t.common.loading}</p>
          ) : variants === 'none' ? (
            <p className="text-xs text-gray-400">{t.catalog.skuNoVariants}</p>
          ) : (() => {
            const sku = skus.find(x => x.id === pickFor)
            if (!sku) return null
            const sorted = [...variants].sort((a, b) => Number(isGuess(sku, b)) - Number(isGuess(sku, a)))
            return (
              <>
                <p className="text-[11px] text-gray-500 dark:text-gray-400 mb-1.5">
                  {sku.name} → {t.catalog.skuPickHint}
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {sorted.slice(0, 40).map(v => {
                    const guess = isGuess(sku, v)
                    const label = v.optionLabel || [v.optionSize, v.optionColor].filter(Boolean).join(' / ') || v.name.slice(0, 28)
                    return (
                      <button key={v.id} onClick={() => patch(sku.id, { supplierProductId: v.id })} title={v.name}
