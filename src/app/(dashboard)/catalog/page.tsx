...
type CatalogItem = {
  ...
  barcode: string
  onlineStockQty: number          // ARICO 온라인샵 재고 (MakeShop/Smaregi 동기화)
...
export default function CatalogPage() {
  ...
  // ARICO 온라인샵 재고 동기화 (MakeShop/Smaregi) — API 연동 전엔 스텁
  const handleSyncStock = async () => {
    setSyncing(true); setSyncMsg(null)
    try {
      const res = await fetch('/api/arico-catalog/sync-stock', { method: 'POST' })
      const d = await res.json()
      if (d.ok) { setSyncMsg(`✅ ${d.updated}/${d.total} ${t.catalog.stockSynced}`); fetchItems(page) }
      else setSyncMsg(`⚠️ ${d.message ?? t.catalog.stockNotConfigured}`)
    } catch { setSyncMsg('⚠️ ' + t.catalog.stockNotConfigured) }
    setSyncing(false)
  }
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={t.catalog.stockSyncTooltip}
          >
      ...
      {syncMsg && (
        <div className="mb-3 text-xs px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300">
          {syncMsg}
        </div>
      )}
        ...
        {items.map(item => {
          ...
          return (
            <div key={item.id} className="relative bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow flex flex-col group">
              ...
              {item.stockSyncedAt && (
                <span className={`absolute top-1 left-1 z-10 px-1.5 py-0.5 rounded text-[10px] font-bold ${item.onlineStockQty > 0 ? 'bg-green-600 text-white' : 'bg-gray-400 text-white'}`} title={t.catalog.onlineStockTitle}>
                  {t.catalog.inStockShort} {item.onlineStockQty}
                </span>
              )}
