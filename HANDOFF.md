...
> 作成 2026-07-14 / 最終更新 **2026-08-11（v1.90.0）**
...
5. **バージョン管理ルール** — デプロイのたびに `src/lib/version.ts` の `APP_VERSION` と `package.json` の `version` を両方上げる（現在 **v1.90.0**）。
...
| 課題 | 状況 |
|---|---|
| **本格稼働日(D-day)の在庫整理** | 🔴 最優先。バックオーダーの在庫表示拡張 →「在庫で充当」ボタン → 過去分の一括再判定。稼働日を決めてから一度で行う方針 |
